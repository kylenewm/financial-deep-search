# Financial Research with Anti-Hallucination Architecture

An **LLM-powered financial research system** that extracts verified facts from SEC filings — extending [LangChain's Open Deep Research](https://github.com/langchain-ai/open_deep_research) with hard verification gates.

## Core Principle

> **"I don't know" is always preferable to "Here's a confident wrong answer"**

Every component is built to **fail closed**:
- Verification fails → return "insufficient data", not a guess
- Extraction ambiguous → reject, don't infer  
- Sources conflict → surface the conflict, don't pick a winner

---

## Hard Context: SEC Pipeline

### Trust Hierarchy

```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│   TIER 1    │ ─► │   TIER 2    │ ─► │   TIER 3    │ ─► │  REJECTED   │
│             │    │             │    │             │    │             │
│  SEC XBRL   │    │  SEC HTML   │    │  Web/News   │    │ Unverified  │
│  Machine    │    │  Verified   │    │  Verified   │    │ Claims      │
│  Readable   │    │  Extraction │    │  Multi-Src  │    │             │
│             │    │             │    │             │    │             │
│ trust=1.0   │    │ trust=0.85  │    │ trust=0.6   │    │ trust=0.0   │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘

INVARIANT: Lower tiers CANNOT override higher tiers
INVARIANT: All tiers require verification before storage
```

### Tier Details

| Tier | Source | Verification | Write to FactStore |
|------|--------|--------------|-------------------|
| **1** | SEC XBRL (structured) | Auto-verified (authoritative) | ✅ Direct |
| **2** | SEC HTML (10-K, 10-Q) | Gate A: sentence must exist in source | ✅ After Gate A |
| **3** | News/Web (Google PSE) | Gate B: cross-check vs FactStore | ✅ After Gate B |

---

## Gate System

### Gate A (Tier 2 → FactStore)

Every LLM-extracted fact must have its `sentence_string` verified to exist in source HTML.

| Fact Type | Verification | Tolerance |
|-----------|--------------|-----------|
| **Numeric** | Extract number from sentence, compare to value | ≤1% exact, ≤5% approx, >5% **REJECTED** |
| **Text** | Normalized substring match in source | Found = pass, Not found = **REJECTED** |

### Gate B (Tier 3 → FactStore)

Cross-verify news claims against existing Tier 1/2 facts.

| Result | Action |
|--------|--------|
| ≤1% match | ✅ Verified |
| ≤5% match | ⚠️ Close (rounding) |
| >5% mismatch | ❌ **Contradicted** — flagged |

---

## Signal Detection (Drift Analysis)

Detect meaningful changes in SEC filing language between periods.

```
Current 10-K (Item 1A)     Previous 10-K (Item 1A)
        │                           │
        └───────────┬───────────────┘
                    ▼
    ┌─────────────────────────────────┐
    ...
    └─────────────────────────────────┘
                    │
                    ▼
    drift_score, new_sentences, removed_sentences, keyword_changes
```

**Use case**: Adding "We may face regulatory action regarding AI products" signals risk before any announcement.

---

## Soft Context: Deep Research Layer

For open-ended qualitative queries (sentiment, market buzz, general research), the system falls back to **LangGraph's multi-agent deep research**:

```
Query → Clarify → Brief → Council Validation → Supervisor → Researchers → Report
                              │                                    │
                    (multi-model consensus)              (Tavily web search)
                                                                   │
                                                          verify_claims()
                                                      (cross-check vs FactStore)
```

| Context Type | Source | Verification | Trust |
|--------------|--------|--------------|-------|
| **Hard** (above) | SEC XBRL, SEC HTML | Gate A/B, pre-write | High |
| **Soft** (this) | Web search, news | Post-hoc vs FactStore | Lower |

**When to use each:**
- Hard context: "What was Apple's Q3 revenue?" → SEC pipeline
- Soft context: "What's the market sentiment on NVDA's AI strategy?" → Deep Research
...
| File | Purpose |
|------|---------|
| `xbrl.py` | SEC EDGAR XBRL extraction — revenue, EPS, assets |
| `extraction.py` | LLM extracts facts with source location pointers |
| `verification.py` | Gate A — validates claims against source text |
| `store.py` | FactStore — single source of truth |
| `narrator.py` | Generates prose from verified facts only |
| `signals.py` | Detects semantic drift in Risk Factors (Item 1A) |
| `orchestrator.py` | Routes queries to appropriate tier handlers |

### Design Invariants

1. **Every fact has a pointer** — location in source document required
2. **LLM proposes, Gate verifies** — two-phase extraction
3. **Narrator reads, never invents** — output grounded in FactStore
4. **Tiered write permissions** — news can't write to FactStore

📖 **Full documentation:** [`docs/ARCHITECTURE_MASTER.md`](docs/ARCHITECTURE_MASTER.md)
