...
class TestResolveEntity:
    ...
    def test_fiscal_year_end_none_when_unknown(self):
        """Test that fiscal_year_end is None for tickers not in FISCAL_YEAR_ENDS."""
        # Find a ticker that exists but isn't in FISCAL_YEAR_ENDS
        # Using a small-cap that's unlikely to be added
        entity = resolve_entity("FIZZ")  # National Beverage Corp
...
class TestGetFiscalYearEnd:
    ...
    def test_unknown_ticker_returns_none(self):
        """Test unknown ticker returns None (not ValueError).
        
        NOTE: Fiscal year end is OPTIONAL metadata. XBRL extraction
        works for ALL tickers without it because the SEC API provides
        fiscal_year/fiscal_period directly.
        """
        result = get_fiscal_year_end("XXXXX")
        assert result is None
    
    def test_unknown_ticker_does_not_raise(self):
        """Test unknown ticker does NOT raise an error."""
        # Should not raise - returns None instead
        result = get_fiscal_year_end("UNKNOWN_TICKER_12345")
        assert result is None
