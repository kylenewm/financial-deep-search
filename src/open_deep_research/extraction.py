...
# Retry configuration
MAX_RETRIES = 0 # 0 retries for now to avoid rate limiting issues
...
async def call_llm_async(prompt: str) -> str:
    """Async version of call_llm for concurrent extraction.
    
    Uses Anthropic AsyncAnthropic with exponential backoff for rate limits.
    ...
    if last_exception:
        raise last_exception
    raise Exception("Unexpected error in async LLM retry loop")
...
async def extract_facts_from_text_async(
    text: str,
    entity: str,
    doc_snapshot: DocumentSnapshot,
    section_id: str,
    ...
    """Async version of extract_facts_from_text for concurrent extraction.
    ...
    # Create prompt and call LLM asynchronously
    prompt = create_text_extraction_prompt(text, entity)
    
    try:
        response = await call_llm_async(prompt)
    except Exception as e:
        logging.error(f"Async LLM extraction failed: {e}")
        return []
...
async def extract_facts_from_table_async(
    table: ExtractedTable,
    entity: str,
    doc_snapshot: DocumentSnapshot
) -> List[Fact]:
    """Async version of extract_facts_from_table for concurrent extraction.
