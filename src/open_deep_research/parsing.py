...
# Suppress XHTML warning - SEC filings are XHTML but HTML parsing works fine
...
def is_toc_line(text: str) -> bool:
    ...
    """
    # Pattern: dots followed by number (page reference) anywhere in the text
...
# =============================================================================
# Part I / Part II Detection (for 10-Q filings)
# =============================================================================


def _find_part_markers(html_content: str) -> List[tuple]:
    """
    Find Part I and Part II markers in a 10-Q filing.
    
    10-Q filings have:
    - Part I: Financial Information (Items 1-4)
    - Part II: Other Information (Items 1-6, different content!)
    
    Returns:
        List of (position, part_number) tuples, sorted by position
    """
    markers = []
    
    # Pattern for Part headers: "PART I", "Part II", etc.
    # Must be after a tag close to avoid matching in attributes
    part_pattern = re.compile(
        r'>[\s\n]*(PART\s+([IV]+|[12]))[\s\n]*<',
        re.IGNORECASE
    )
    
    for match in part_pattern.finditer(html_content):
        part_text = match.group(2).upper()
        
        # Convert Roman numerals to numbers
        if part_text in ('I', '1'):
            part_num = 'I'
        elif part_text in ('II', '2'):
            part_num = 'II'
        elif part_text in ('III', '3'):
            part_num = 'III'
        elif part_text in ('IV', '4'):
            part_num = 'IV'
        else:
            continue
        
        # Check if this is in TOC (skip it)
        start_ctx = max(0, match.start() - 200)
        context = html_content[start_ctx:match.start()].lower()
        if 'table of contents' in context or '.....' in context:
            continue
        
        markers.append((match.start(), part_num))
...
def _get_part_for_position(position: int, part_markers: List[tuple]) -> Optional[str]:
    """
    Determine which Part a position falls under.
    
    Args:
        position: Character position in the HTML
        part_markers: List of (position, part_number) from _find_part_markers
        
    Returns:
        Part number ('I', 'II', etc.) or None if before any Part marker
    """
    current_part = None
    
    for marker_pos, part_num in part_markers:
...
def chunk_by_section(html_content: str) -> List[Section]:
    ...
    for match in header_pattern.finditer(html_content):
        ...
        # Check if this specific matched text is a TOC line
        # (dots followed by page number in the same line)
        matched_text = match.group(0)  # The full matched string
        matched_text_clean = re.sub(r'<[^>]+>', ' ', matched_text)
        if is_toc_line(matched_text_clean):
            continue
    ...
    # Detect Part I vs Part II markers for 10-Q filings
    # This is critical: 10-Q has Item 1-4 in BOTH Part I and Part II
    # Part I Item 1 = Financial Statements (what we want)
    # Part II Item 1 = Legal Proceedings (different content)
    part_markers = _find_part_markers(html_content)
    
    # Assign part numbers to each match based on position
    for m in matches:
        m['part'] = _get_part_for_position(m['start'], part_markers)
        # Create unique ID: "PartI_Item2" or "PartII_Item2"
        if m['part']:
            m['full_section_id'] = f"Part{m['part']}_{m['section_id']}"
        else:
            m['full_section_id'] = m['section_id']
    
    # Deduplicate by full_section_id (Part + Item), keep FIRST occurrence
    # First occurrence after TOC is the real content
    seen = {}
    for m in matches:
        if m['full_section_id'] not in seen:
            seen[m['full_section_id']] = m  # Keep first occurrence
...
def extract_risk_factors(parsed: "ParsedFiling") -> Optional[str]:
    """
    Extract Item 1A Risk Factors text from a parsed filing.
    
    .. deprecated::
        Use :func:`section_locator.extract_risk_factors_from_html` instead.
        That function provides layered extraction (TOC → DOM → fuzzy) with
        explicit confidence levels, which is more robust than regex-based parsing.
    
    Risk Factors (Item 1A) is the key section for signal detection,
    as it contains management's disclosure of material risks.
    
    Args:
        parsed: ParsedFiling from parse_filing_html()
        
    Returns:
        Risk Factors section text, or None if not found
    """
    warnings.warn(
        "extract_risk_factors(parsed) is deprecated. "
        "Use section_locator.extract_risk_factors_from_html(html) instead for "
        "layered extraction with explicit confidence levels.",
        DeprecationWarning,
        stacklevel=2,
    )
    for section in parsed.sections:
        if section.section_id == "Item1A":
            return section.text
    return None
