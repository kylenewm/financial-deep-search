...
def verify_text_fact(fact: Fact, source_text: str) -> Fact:
    ...
    For source_format == "html_text" or "news".
    ...
    Args:
        fact: The Fact to verify (must have source_format == "html_text" or "news")
    ...
    Raises:
        ValueError: If fact is not a text or news fact
    """
    if fact.source_format not in ("html_text", "news"):
...
def process_extracted_facts(
    ...
    for fact in facts:
        ...
        elif fact.source_format == "news":
            # News facts use the text verification path
            verified_fact = verify_text_fact(fact, source_text)
        elif fact.source_format == "xbrl":
            # XBRL facts are pre-verified by construction
            verified_fact = fact
        else:
            fact.verification_status = "mismatch"
            verified_fact = fact
...
# =============================================================================
# Async Extraction Pipeline (Concurrent LLM Calls)
# =============================================================================


# Concurrency settings - ultra conservative for 50 RPM rate limit
# Sequential processing with 1.5s delay = 30 req/min (safely under 50)
CONCURRENT_LLM_CALLS = 1  # Sequential - no concurrency
BATCH_SIZE = 1  # One request at a time
BATCH_DELAY = 1.5  # 1.5s between requests = 30 req/min max


async def extraction_pipeline_async(
    section: Section,
    entity: str,
    doc_snapshot: DocumentSnapshot
) -> Tuple[List[Fact], List[Fact]]:
    """Async version with concurrent LLM calls for faster extraction.
    
    Uses semaphore to limit concurrent API calls and avoid rate limits.
    ...
    import asyncio
    from open_deep_research.extraction import (
        extract_facts_from_text_async,
        extract_facts_from_table_async,
    )
    
    all_facts = []
    
    # Extract tables from section (sync - no LLM calls)
    tables = extract_tables_from_section(section)
    
    # Create semaphore for rate limiting
    semaphore = asyncio.Semaphore(CONCURRENT_LLM_CALLS)
    
    async def extract_para_with_limit(para):
        """Extract from paragraph with concurrency limit."""
        async with semaphore:
            return await extract_facts_from_text_async(
                text=para.text,
                entity=entity,
                doc_snapshot=doc_snapshot,
                section_id=section.section_id,
    ...
    async def extract_table_with_limit(table):
        """Extract from table with concurrency limit."""
        async with semaphore:
            return await extract_facts_from_table_async(
                table=table,
                entity=entity,
                doc_snapshot=doc_snapshot
            )
    
    # Process paragraphs in batches with delays to avoid rate limits
    para_facts = []
    num_batches = (len(section.paragraphs) + BATCH_SIZE - 1) // BATCH_SIZE
    
    for i, batch_start in enumerate(range(0, len(section.paragraphs), BATCH_SIZE)):
        batch = section.paragraphs[batch_start:batch_start + BATCH_SIZE]
        tasks = [extract_para_with_limit(p) for p in batch]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        for result in results:
            if isinstance(result, Exception):
                logger.warning(f"Paragraph extraction failed: {result}")
            elif result:
                para_facts.extend(result)
        
        # Add delay between batches (except after last batch)
        if i < num_batches - 1 and BATCH_DELAY > 0:
            await asyncio.sleep(BATCH_DELAY)
    
    all_facts.extend(para_facts)
    
    # Process tables concurrently
    if tables:
        table_tasks = [extract_table_with_limit(t) for t in tables]
        table_results = await asyncio.gather(*table_tasks, return_exceptions=True)
        
        for result in table_results:
            if isinstance(result, Exception):
                logger.warning(f"Table extraction failed: {result}")
            elif result:
    ...
    # Process through verification gate (sync)
    verified, rejected = process_extracted_facts(all_facts, source_text, tables)
    
    logger.info(
        f"Section {section.section_id} (async): "
        f"{len(verified)} verified, {len(rejected)} rejected, "
        f"{len(tables)} tables"
    )
